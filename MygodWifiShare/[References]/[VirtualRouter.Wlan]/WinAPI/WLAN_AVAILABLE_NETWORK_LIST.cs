...
namespace VirtualRouter.Wlan.WinAPI
{
    ...
    internal struct WLAN_AVAILABLE_NETWORK_LIST
    {
        ...
        internal WLAN_AVAILABLE_NETWORK_LIST(IntPtr ppAvailableNetworkList)
        {
            ...
            {
                var pWlanAvailableNetwork =
                    new IntPtr(ppAvailableNetworkList.ToInt32() + i * Marshal.SizeOf(typeof(WLAN_AVAILABLE_NETWORK)) + 8);
                wlanAvailableNetwork[i] =
                    (WLAN_AVAILABLE_NETWORK) Marshal.PtrToStructure(pWlanAvailableNetwork, typeof(WLAN_AVAILABLE_NETWORK));
            }
        }
    }
}